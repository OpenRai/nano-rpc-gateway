...
function codeBlock(label, value) {
  return `<section class="example"><div class="example-head"><h3>${label}</h3><button class="copy" type="button">Copy</button></div><pre><code>${escapeHtml(value)}</code></pre></section>`
}
...
<div class="tabs" role="tablist" aria-label="Reference section"><button class="tab active" data-tab="rpc" role="tab" aria-controls="rpc" aria-selected="true">RPC <span>${rpc.length}</span></button><button class="tab" data-tab="event" role="tab" aria-controls="event" aria-selected="false">Events <span>${events.length}</span></button></div>
